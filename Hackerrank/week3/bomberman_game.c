 ...
 *
 */
char **fullGrid(int rows, int cols)
{
...
{
    char **result = fullGrid(rows, cols);
...
char **bomberMan(int n, int grid_count, char **grid, int *result_count)
{
    ...
    *result_count = rows;
    ...
    if (n % 2 == 0)
    {
        return fullGrid(rows, cols); // full bombs
    }

    // First detonation (time = 3)
    char **stateA = detonate(rows, cols, grid);
    if (n % 4 == 3)
    {
        return stateA;
    }

    // Second detonation (time = 5, 9, 13,...)
    char **stateB = detonate(rows, cols, stateA);
    return stateB;
}
